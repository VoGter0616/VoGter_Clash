...
> 默认关闭
> 
> 如果使用按照以下设置，且必须保证节点支持 IPv6 和路由 IPv6 的设置正确。如果节点不支持 IPv6 还需要使用，设置好路由的 IPv6 后仅开启“允许 IPv6 类型 DNS 解析”。
...
- 启用 TCP 并发 ✅
- 启用统一延迟 ✅
- Geodata 数据加载方式：**标准模式**（根据设备的内存而定，设备大内存使用标准模式，小内存使用低内存模式或者直接禁用）
- 启用流量（域名）探测 ✅
- 探测（嗅探）纯 IP 连接 ✅
- 自定义流量探测（嗅探）设置 ✅
...
> 全局代理模式（Proxy）下 DNS 通常不走代理，只有配置（Config）模式下才可能正确走代理。如果你更在意 DNS 防泄漏，建议使用配置模式，并正确设置 `dns-server` 和 `proxy-dns-serve` 参数。
...
> [!TIP]
> 如果需要使用特定 DNS 解析域名，在 **配置 → 本地文件** 里选择 `Shadowrocket_basic.conf`，长按选择 **编辑纯文本**，根据标签提示修改`proxy-dns-serve` 参数及翻阅最下方`[Host]` 参数。如果无特殊需求，无视此操作。
...
- 以 **Connect 测速** 结果为准判断节点可用性。
- 确保 Shadowrocket 版本在 **2.2.92** 或更高，旧版本对 AnyTLS 支持不完善。
...
> [!WARNING]
> 本设置适用于初次使用设置及完整的配置文件适配6.9.0，后续不会再对Surge配置进行维护，请自行斟酌使用。
...
1. 按照通配符的书写方式，把机场 AnyTLS 协议节点的域名和 DNS 按照上述格式进行复制粘贴并修改。
